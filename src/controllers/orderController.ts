...
export async function createOrder(req: Request, res: Response) {
  try {
    ...
    if (!tenantId || !tableName || !items || !Array.isArray(items) || items.length === 0 || !total) {
...
export async function updateOrderStatus(req: Request, res: Response) {
  try {
    ...
    if (!status || !validStatuses.includes(status)) {
      return res.status(400).json({ error: 'Invalid status. Must be one of: PENDING, CONFIRMED, DONE, CANCELLED' });
    }
